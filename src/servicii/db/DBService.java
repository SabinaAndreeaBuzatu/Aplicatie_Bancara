...
public class DBService {
    ...
    public void addCutie(InchiriereCutieValori c) throws SQLException {
        ...
        try {
            statement.execute("insert into cutie values (\"" + c.getNrCutie() + "\",\"" + c.getContc() + "\",\"" + c.getPragValoriDeclarat() + "\",\"" + c.getDataAcordare() + "\",\"" + c.getAdresa() + "\",\"" + c.getValuta() + "\")");
    ...
    public void addClient(Client c) throws SQLException {
        ...
        Statement statement = connect.createStatement();
        try {
            statement.execute("insert into client values (\"" + c.getCNP() + "\",\"" + c.getNume() + "\",\"" + c.getPrenume() + "\",\"" + c.getDataNasterii() + "\")");
    ...
    public void addCard(List<Cont> cont, Card c) throws SQLException {
        ...
        try {
            ...
            System.out.println("Cardul exista deja! :(");
        }
    ...
    public void updateClient(Client c) throws SQLException {
        ...
        Statement statement = connect.createStatement();
        Statement statement2 = connect.createStatement();
        ResultSet rs = statement2.executeQuery("select count(*) from client where CNP like " + c.getCNP());
        if (rs.next()) {
            if (rs.getInt(1) == 0)
                System.out.println("CNP nu corespunde cu un CNP existent in baza de date. Clientul nu exista!");
            else {

                try {
                    statement.execute("update client set  Nume = \"" + c.getNume() + "\",Prenume = \"" + c.getPrenume() + "\",data_nasterii= \"" + c.getDataNasterii() + "\" where cnp like '" + c.getCNP() + "'");
                    System.out.println("Update efectuat! Client");
                } catch (SQLSyntaxErrorException | SQLIntegrityConstraintViolationException e) {
                    System.out.println("Eroare de sintaxa sau cv");
                }
    ...
    public void updateCont(ContCurent c) throws SQLException {
        ...
        Statement statement = connect.createStatement();
        Statement statement2 = connect.createStatement();
        ResultSet rs = statement2.executeQuery("select count(*) from contcurent where IBAN like '" + c.getIBAN()+"'");
        if (rs.next()) {
            if (rs.getInt(1) == 0)
                System.out.println("IBAN-ul nu corespunde cu un IBAN existent in baza de date. Contul nu exista!");
            else {
        try {
            statement.execute("update contcurent set CNP= \"" + c.getCNP() + "\", Valuta=  \"" + c.getValuta() + "\",sold= \"" + c.getSold() + "\",data_acordare=\"" + c.getDataAcordare() + "\",tip_cont=\"" + c.gettipcont() + "\",comision=\"" + c.getComisionA() + "\" where IBAN like '" + c.getIBAN() + "'");
    ...
    public void updateDepozit(Depozit c) throws SQLException {
        ...
        Statement statement = connect.createStatement();
        Statement statement2 = connect.createStatement();
        ResultSet rs = statement2.executeQuery("select count(*) from depozit where IBAN like '" + c.getIBAN()+"'");
        if (rs.next()) {
            if (rs.getInt(1) == 0)
                System.out.println("IBAN-ul nu corespunde cu un IBAN existent in baza de date. Depozitul nu exista!");
            else {
                try {
                    statement.execute("update depozit set IBANC= \"" + c.getIBANcontc() + "\", Valuta=  \"" + c.getValuta() + "\",sold= \"" + c.getSold() + "\",data_acordare=\"" + c.getDataAcordare() + "\",tip_cont=\"" + c.gettipcont() + "\",comision=\"" + c.getComisionA() + "\", dobanda=\"" + c.getDobanda() + "\" where IBAN like '" + c.getIBAN()+"'");
    ...
    public void updateCredit(Credit c) throws SQLException {
        ...
        Statement statement = connect.createStatement();
        Statement statement2 = connect.createStatement();
        ResultSet rs = statement2.executeQuery("select count(*) from credit where IBAN like '" + c.getIBAN()+"'");
        if (rs.next()) {
            if (rs.getInt(1) == 0)
                System.out.println("IBAN-ul nu corespunde cu un IBAN existent in baza de date. Creditul nu exista!");
            else {
        try {
            statement.execute("update credit set IBANC= \"" + c.getIBANcontc() + "\", Valuta=  \"" + c.getValuta() + "\",sold= \"" + c.getSold() + "\",data_acordare=\"" + c.getDataAcordare() + "\",tip_cont=\"" + c.gettipcont() + "\",comision=" + c.getComisionA() + ", dobanda=" + c.getDobanda() + ",durataLuni=" + c.getDurataluni() + ",rata=" + c.getRatac() + ",ratePlatite=" + c.getRatePlatite() + " where IBAN like '" + c.getIBAN()+"'");
  ...
  public void deleteallCutie() throws SQLException { Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
      Statement statement = connect.createStatement();
      statement.execute("delete from cutie");}
    public void deleteallCredit() throws SQLException { Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
        statement.execute("delete from credit");}
    public void deleteallDepozit() throws SQLException { Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
        statement.execute("delete from depozit");}
    public void deleteallContCurent() throws SQLException { Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
        statement.execute("delete from contcurent");}
    public void deleteallClient() throws SQLException { Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
        statement.execute("delete from client");}
    public void clearDB() throws SQLException { deleteallClient();
        }
    ...
    public void deleteCont(ContCurent c) throws SQLException {
        Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
        statement.execute("delete from contcurent where IBAN like \"" + c.getIBAN() + "\"");
    }

    public void deleteCard(Card c) throws SQLException {
        Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
        statement.execute("delete from card where IBAN like \"" + c.getNrCard() + "\"");
    }

    public void deleteDepozit(Depozit c) throws SQLException {
        Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
        statement.execute("delete from depozit where IBAN like \"" + c.getIBAN() + "\"");
    }

    public void deleteCredit(Credit c) throws SQLException {
        Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
        statement.execute("delete from credit where IBAN like \"" + c.getIBAN() + "\"");
    }

    public void deleteCutie(InchiriereCutieValori c) throws SQLException {
        Connection connect = DriverManager.getConnection("jdbc:mysql://localhost:3306/banca", "root", "root-pass");
        Statement statement = connect.createStatement();
